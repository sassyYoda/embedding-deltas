...
- **D-75 (RE-AMENDED 2026-05-06):** **Phase 5 tunes parameters on `tiger_woods` (27.5 min) — the longest USABLE video after marcus_jordan was dropped.** Background:
  - Spec §6: "Tune these once on the **longest/most representative** video..."
  - **`marcus_jordan` (75.2 min) was the longest** but exhibited a reproducible MPS hang/crash during its CLIP extraction — failed cleanly at ~50% on chunked attempt, hung in `MPSStream::executeMPSGraph` deadlock on streaming retry. After the second hang during a parallel JT smoke run, scope was narrowed to drop marcus_jordan.
  - **`tiger_woods` (27.5 min, 3,297 frames) is the next-longest** — 1.5× JT's length, signal-rich, embedding fixture already on disk from background extracts, no MPS issues observed during its successful 22.7-min extract.
  - **Tuning sequence:** (1) Run `pipeline.py videos/tiger_woods.mp4` (re-extracts CLIP via the canonical entry point — ~22 min wall-clock on solo MPS). (2) Assess peak count (target 3-30 per spec §6 advice), budget fill %, qualitative reel quality. (3) If parameters need adjustment from defaults `(--height=1.5, --min-gap-sec=15, --merge-gap-sec=3)`, sweep them on tiger_woods. (4) Freeze the values that produced the best tiger_woods reel; those become the argparse defaults Phase 6 uses.
  - **JT smoke test deferred** — both JT smoke and marcus extract hung today; running both simultaneously poisoned MPS state. Tiger Woods solo run validates the full pipeline.py end-to-end (frame sampling → CLIP → signal → clip → export → JSON) AND tunes parameters in one shot.
